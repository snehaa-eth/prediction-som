...
import { usePredictionTx, useMarketDetail } from '../hooks/useContract';
...
export const MarketDetailScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const { market: routeMarket } = route.params as { market: Market };
  const marketId = Number(routeMarket.id);

  // Live data from chain (falls back to route param while loading)
  const { market: liveMarket } = useMarketDetail(marketId);
  const market = liveMarket ?? routeMarket;
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await buyOutcome(marketId, outcome as 0 | 1, String(amount));
      const msg = `Trade Placed! You predicted ${pendingPrediction} with ${amount} TFY`;
      if (Platform.OS === 'web') {
        window.alert(msg);
      } else {
        Alert.alert('Trade Placed!', msg);
      }
    } catch (e: any) {
      const msg = e.message?.slice(0, 100) ?? 'Transaction failed';
      if (Platform.OS === 'web') {
        window.alert(`Trade failed: ${msg}`);
      } else {
        ...
        <View style={styles.centered}>
          <View style={styles.header}>
            ...
            <View style={styles.headerActions}>
              <View style={[styles.iconBtn, { backgroundColor: theme.accentDim, borderColor: theme.accent }]}>
                <Text style={[styles.iconText, { color: theme.accent }]}>#{market.id}</Text>
              </View>
            </View>
          </View>
          ...
          <View>
            <Text style={[styles.amountTitle, { color: theme.textSecondary }]}>Select Amount (TFY)</Text>
            <ScrollView
              horizontal
          ...
          {market.timeLeft !== 'Resolved' && (
            <View style={styles.tradeSection}>
              <Text style={[styles.tradeTitle, { color: theme.accent }]}>
                {txPending ? 'Submitting Transaction...' : 'Swipe to Predict'}
              </Text>
              <SwipeTradeCard
                amount={amount}
          ...
          {market.timeLeft === 'Resolved' && (
            <View style={[styles.resolvedBanner, { backgroundColor: theme.cardBg, borderColor: theme.border }]}>
              <Text style={[styles.resolvedText, { color: theme.textSecondary }]}>This market has been resolved</Text>
            </View>
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  backBtn: { width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center', borderWidth: 1, cursor: 'pointer' as any },
  backText: { fontSize: 20 },
  headerActions: { flexDirection: 'row', gap: 8 },
  iconBtn: { paddingHorizontal: 12, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center', borderWidth: 1, cursor: 'pointer' as any },
  iconText: { fontSize: 14, fontWeight: '600' },
  tag: { alignSelf: 'flex-start', paddingHorizontal: 8, paddingVertical: 4, borderRadius: radii.sm, marginBottom: 12 },
  tagText: { fontSize: 11, fontWeight: '500' },
  question: { fontSize: 22, fontWeight: '600', marginBottom: 20, lineHeight: 30 },
  probRow: { flexDirection: 'row', borderRadius: radii.card, borderWidth: 1, marginBottom: 20, overflow: 'hidden' },
  probBox: { flex: 1, alignItems: 'center', paddingVertical: 20 },
  probLabel: { fontSize: 13, fontWeight: '500', marginBottom: 4 },
  probValue: { fontSize: 28, fontWeight: '700' },
  probDivider: { width: 1 },
  chartLabel: { fontSize: 13, fontWeight: '500', marginBottom: 10 },
  infoRow: { flexDirection: 'row', marginTop: 20, marginBottom: 24, gap: 12 },
  infoItem: { flex: 1, borderRadius: radii.button, padding: 14, borderWidth: 1, alignItems: 'center' },
  infoLabel: { fontSize: 11, fontWeight: '500', marginBottom: 6 },
  infoValue: { fontSize: 13, fontWeight: '600' },
  amountTitle: { fontSize: 13, fontWeight: '500', marginBottom: 10 },
  amountRow: { gap: 8, paddingBottom: 4 },
  amountChip: { paddingHorizontal: 20, paddingVertical: 10, borderRadius: 20, borderWidth: 1, cursor: 'pointer' as any },
  amountChipText: { fontSize: 13, fontWeight: '500' },
  tradeSection: { marginTop: 24 },
  tradeTitle: { fontSize: 16, fontWeight: '600', textAlign: 'center', marginBottom: 16, textTransform: 'uppercase', letterSpacing: 2 },
  resolvedBanner: { marginTop: 24, borderRadius: radii.card, padding: 20, borderWidth: 1, alignItems: 'center' },
  resolvedText: { fontSize: 15, fontWeight: '500' },
});
